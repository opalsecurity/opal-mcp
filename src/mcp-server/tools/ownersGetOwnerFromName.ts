/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import { ownersGetOwnerFromName } from "../../funcs/ownersGetOwnerFromName.js";
import * as operations from "../../models/operations/index.js";
import { formatResult, ToolDefinition } from "../tools.js";

const args = {
  request: operations.GetOwnerFromNameRequest$inboundSchema,
};

export const tool$ownersGetOwnerFromName: ToolDefinition<typeof args> = {
  name: "owners-get-owner-from-name",
  description:
    `Returns an \`Owner\` object. Does not support owners with \`/\` in their name, use /owners?name=... instead.`,
  args,
  tool: async (client, args, ctx) => {
    const [result, apiCall] = await ownersGetOwnerFromName(
      client,
      args.request,
      { fetchOptions: { signal: ctx.signal } },
    ).$inspect();

    if (!result.ok) {
      return {
        content: [{ type: "text", text: result.error.message }],
        isError: true,
      };
    }

    const value = result.value;

    return formatResult(value, apiCall);
  },
};
