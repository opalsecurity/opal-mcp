 ...
 */
export function resourcesResourceUserAccessStatusRetrieve(
  client: OpalMcpCore,
  request: operations.ResourceUserAccessStatusRetrieveRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ResourceUserAccessStatus,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ResourceUserAccessStatusRetrieveRequest$outboundSchema.parse(
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ResourceUserAccessStatus$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
