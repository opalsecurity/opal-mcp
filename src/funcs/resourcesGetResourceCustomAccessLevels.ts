 ...
 */
export function resourcesGetResourceCustomAccessLevels(
  client: OpalMcpCore,
  request: operations.GetResourceCustomAccessLevelsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ResourceCustomAccessLevelList,
    | OpalMcpError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetResourceCustomAccessLevelsRequest$outboundSchema.parse(
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ResourceCustomAccessLevelList$inboundSchema),
    M.fail([400, 404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
