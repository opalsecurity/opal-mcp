 ...
 */
export function resourcesGetResourceGroups(
  client: OpalMcpCore,
  request: operations.GetResourceGroupsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.GroupResourceList,
    | OpalMcpError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetResourceGroupsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/resources/{resource_id}/groups")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.GroupResourceList$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
