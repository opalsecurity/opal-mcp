 ...
 */
export function opalQueriesRunOpalQuery(
  client: OpalMcpCore,
  request: components.RunOpalQueryRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.OpalQueryResults,
    | OpalMcpError
...
> {
  const parsed = safeParse(
    request,
    (value) => components.RunOpalQueryRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/queries/run")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.OpalQueryResults$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
