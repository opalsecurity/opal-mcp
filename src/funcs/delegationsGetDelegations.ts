 ...
 */
export function delegationsGetDelegations(
  client: OpalMcpCore,
  request: operations.GetDelegationsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.PaginatedDelegationsList,
    | OpalMcpError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetDelegationsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/delegations")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.PaginatedDelegationsList$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
