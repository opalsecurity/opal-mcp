 ...
 */
export function resourcesUpdateResourceCustomAccessLevel(
  client: OpalMcpCore,
  request: operations.UpdateResourceCustomAccessLevelRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ResourceCustomAccessLevelResponse,
    | OpalMcpError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateResourceCustomAccessLevelRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload.UpdateResourceCustomAccessLevelInfo, {
    explode: true,
  });

  const pathParams = {
    access_level_remote_id: encodeSimple(
      "access_level_remote_id",
      payload.access_level_remote_id,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ResourceCustomAccessLevelResponse$inboundSchema),
    M.fail([400, 404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
