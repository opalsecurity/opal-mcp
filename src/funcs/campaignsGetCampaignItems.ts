 ...
 */
export function campaignsGetCampaignItems(
  client: OpalMcpCore,
  request: operations.GetCampaignItemsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.PaginatedCampaignItemsList,
    | OpalMcpError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetCampaignItemsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/campaigns/{campaign_id}/items")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.PaginatedCampaignItemsList$inboundSchema),
    M.fail([404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
