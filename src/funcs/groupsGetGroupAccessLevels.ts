 ...
 */
export function groupsGetGroupAccessLevels(
  client: OpalMcpCore,
  request: operations.GetGroupAccessLevelsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.GroupAccessLevelList,
    | OpalMcpError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetGroupAccessLevelsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/groups/{group_id}/access_levels")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.GroupAccessLevelList$inboundSchema),
    M.fail([404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
