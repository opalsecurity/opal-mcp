 ...
 */
export function accessRulesCreateAccessRule(
  client: OpalMcpCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => components.UpdateAccessRuleInfo$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.AccessRule$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
