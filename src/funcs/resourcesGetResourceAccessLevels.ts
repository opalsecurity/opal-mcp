 ...
 */
export function resourcesGetResourceAccessLevels(
  client: OpalMcpCore,
  request: operations.GetResourceAccessLevelsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ResourceAccessLevelList,
    | OpalMcpError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetResourceAccessLevelsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/resources/{resource_id}/access_levels")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ResourceAccessLevelList$inboundSchema),
    M.fail([404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
