 ...
 */
export function groupsAddGroupResource(
  client: OpalMcpCore,
  request: operations.AddGroupResourceRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.GroupResource,
    | OpalMcpError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.AddGroupResourceRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/groups/{group_id}/resources/{resource_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.GroupResource$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
