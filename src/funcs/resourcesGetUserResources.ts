 ...
 */
export function resourcesGetUserResources(
  client: OpalMcpCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetUserResourcesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/resources/users/{user_id}")(pathParams);

  const query = encodeFormQuery({
    "cursor": payload.cursor,
    "include_unmanaged": payload.include_unmanaged,
    "limit": payload.limit,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ResourceAccessUserList$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
