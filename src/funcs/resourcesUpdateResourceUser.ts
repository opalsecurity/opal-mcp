 ...
 */
export function resourcesUpdateResourceUser(
  client: OpalMcpCore,
  request: operations.UpdateResourceUserRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ResourceUser,
    | OpalMcpError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UpdateResourceUserRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/resources/{resource_id}/users/{user_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.ResourceUser$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
