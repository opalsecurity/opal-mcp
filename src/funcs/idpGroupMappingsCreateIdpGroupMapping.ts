 ...
 */
export function idpGroupMappingsCreateIdpGroupMapping(
  client: OpalMcpCore,
  request: operations.CreateIdpGroupMappingRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.IdpGroupMapping,
    | OpalMcpError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateIdpGroupMappingRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.IdpGroupMapping$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
