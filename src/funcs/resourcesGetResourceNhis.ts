 ...
 */
export function resourcesGetResourceNhis(
  client: OpalMcpCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetResourceNhisRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/resources/{resource_id}/non-human-identities")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.AccessList$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
