 ...
 */
export function resourcesAddResourceNhi(
  client: OpalMcpCore,
  request: operations.AddResourceNhiRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.ResourceNHI,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.AddResourceNhiRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    non_human_identity_id: encodeSimple(
      "non_human_identity_id",
      payload.non_human_identity_id,
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ResourceNHI$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
