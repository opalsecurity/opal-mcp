 ...
 */
export function ownersSetOwnerUsers(
  client: OpalMcpCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.SetOwnerUsersRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.UserIDList, { explode: true });

  const pathParams = {
    owner_id: encodeSimple("owner_id", payload.owner_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/owners/{owner_id}/users")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.UserList$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
