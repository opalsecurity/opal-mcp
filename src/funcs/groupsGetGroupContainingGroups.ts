...
export function groupsGetGroupContainingGroups(
  ...
  Result<
    components.GroupContainingGroupList,
    | OpalMcpError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetGroupContainingGroupsRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.GroupContainingGroupList$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
