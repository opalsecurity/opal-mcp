 ...
 */
export type ReviewerStages = Array<RequestReviewerStages> | string;
...
export type RequestT = {
   ...
   * The configured reviewer stages for every item in this request, or an error message if reviewers could not be loaded
   */
  reviewerStages?: Array<RequestReviewerStages> | string | undefined;
};

/** @internal */
export const ReviewerStages$inboundSchema: z.ZodType<
  ReviewerStages,
  z.ZodTypeDef,
  unknown
> = z.union([z.array(RequestReviewerStages$inboundSchema), z.string()]);
/** @internal */
export type ReviewerStages$Outbound =
  | Array<RequestReviewerStages$Outbound>
  | string;

/** @internal */
export const ReviewerStages$outboundSchema: z.ZodType<
  ReviewerStages$Outbound,
  z.ZodTypeDef,
  ReviewerStages
> = z.union([z.array(RequestReviewerStages$outboundSchema), z.string()]);

export function reviewerStagesToJSON(reviewerStages: ReviewerStages): string {
  return JSON.stringify(ReviewerStages$outboundSchema.parse(reviewerStages));
}
export function reviewerStagesFromJSON(
  jsonString: string,
): SafeParseResult<ReviewerStages, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => ReviewerStages$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ReviewerStages' from JSON`,
  );
}
...
> = z.object({
  ...
  reviewer_stages: z.union([
    z.array(RequestReviewerStages$inboundSchema),
    z.string(),
  ]).optional(),
}).transform((v) => {
  return remap$(v, {
...
export type RequestT$Outbound = {
  ...
  reviewer_stages?: Array<RequestReviewerStages$Outbound> | string | undefined;
};
...
> = z.object({
  ...
  reviewerStages: z.union([
    z.array(RequestReviewerStages$outboundSchema),
    z.string(),
  ]).optional(),
}).transform((v) => {
  return remap$(v, {
